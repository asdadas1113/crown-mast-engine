...
class WaveAStudyDesignTests(unittest.TestCase):
    def test_verified_core_allowlists_exclude_open_model_risks(self) -> None:
        selected = {
            *WAVE_A_B1_CANDIDATES,
            *WAVE_A_MAIN_B3_CANDIDATES,
            *WAVE_A_SECONDARY_B3_ANCHORS,
        ...
        self.assertNotIn("moran-favorite-item", selected)
        self.assertNotIn("scarlet-black-shadow", selected)
        self.assertNotIn("liberalio", selected)
    ...
    def test_roster_arithmetic_and_duplicate_policy(self) -> None:
        self.assertEqual(WAVE_A_RAW_ROSTER_COUNT, 72)
        ...
        self.assertEqual(WAVE_A_VALID_ROSTER_COUNT, 69)
    ...
    def test_environment_and_total_case_arithmetic(self) -> None:
        ...
        self.assertEqual(WAVE_A_SCENARIO_COUNT, 13_248)

    def test_current_allowlist_is_certified_below_reload_speed_cap(self) -> None:
    ...
    def test_blocked_roster_is_rejected_fail_closed(self) -> None:
        blocked = TeamRoster(main_b3="liberalio")
        with self.assertRaisesRegex(ValueError, "outside the Wave A verified-core sample"):
            build_wave_a_roster_cases(blocked)

    def test_definition_is_self_consistent_and_draft_only(self) -> None:
        definition = wave_a_study_definition()
        self.assertEqual(definition["valid_roster_count"], 69)
        self.assertEqual(definition["scenario_count"], 13_248)
